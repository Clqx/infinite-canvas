...
```text
<画布网页地址>/canvas?mode=new#agentUrl=<encoded Local URL>&agentToken=<encoded Connect token>
```

`mode=new` 会让网页自动创建具体画布并连接本地 Agent，不需要手动点击“新建画布”。连接地址和 token 必须先进行 URL 编码并放在 fragment 中，网页读取后会立即从地址栏移除，不能放入查询参数。
