...
+ [新增] 新增本地用户档案、管理员角色与用户管理，并按用户隔离凭据、画布、素材、媒体、生成记录和同步目录；新增一次性激活码、当前用户标识、档案版本与跨标签停用同步。
+ [调整] 生产 Docker 默认启用 fail-closed Basic Auth 与安全响应头，本地无鉴权模式仅允许显式回环部署。
+ [修复] Canvas Agent 改为 header-only token 鉴权并限制工作区文件访问，启动链接中的连接凭据改由 fragment 一次性导入和清除。
+ [修复] 本地业务数据改为可等待、可重试且可见的持久化流程，并补齐项目切换隔离与媒体引用保护，避免刷新、并发写入或清理时丢失数据。
+ [新增] 配置页新增完整备份导出、无写入恢复演练和副本导入，备份包含已持久化业务数据、删除记录及全部引用媒体，并支持中断后继续或回退。
+ [修复] WebDAV 同步增加严格清单、媒体内容校验、删除记录合并和四数据域统一快照提交，遇到远端版本、容量或提交冲突时停止或重新合并。
